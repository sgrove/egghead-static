...
function checkDoIHaveARepo(client, repoName, username) {
  return $$Promise.flatMap(GraphQL$EggheadStatic.query(client, GraphQL$EggheadStatic.DoIHaveARepoQuery.make(username, repoName, /* () */0), /* NetworkOnly */971373850, /* () */0), (function (data) {
                ...
                if (data.tag) {
                  ...
                  var match = data[0].response;
                  if (typeof match === "number") {
                    console.warn("NotFound error for getFileSha");
                    return $$Promise.resolved(false);
                  } else if (match.tag) {
                    console.warn("Error getting data response: ", match[0]);
                    return $$Promise.resolved(false);
                  } else {
                    return $$Promise.resolved(Belt_Option.isSome(Belt_Option.map(Belt_Option.flatMap(match[0].gitHub, (function (gitHub) {
                                              return gitHub.repository;
                                            })), (function (repo) {
                                          return repo.id;
                                        }))));
...
function forkRepo(client, repoOwner, repoName) {
  return GraphQL$EggheadStatic.mutation(client, GraphQL$EggheadStatic.ForkGitHubRepoMutation.make(repoOwner, repoName, /* () */0), "Error forking source repository");
}

function getFileShaAndContent(client, repoName, repoOwner, branchAndFilePath) {
  return $$Promise.flatMap(GraphQL$EggheadStatic.query(client, GraphQL$EggheadStatic.GetFileShaAndContentQuery.make(repoName, repoOwner, branchAndFilePath, /* () */0), undefined, /* () */0), (function (promise) {
                if (promise.tag) {
                  console.warn("Error getting data response: ", promise[0]);
                  return $$Promise.resolved(undefined);
                } else {
                  var match = promise[0].response;
                  if (typeof match === "number") {
                    console.warn("NotFound error for getFileSha");
                    return $$Promise.resolved(undefined);
                  } else if (match.tag) {
                    ...
                    return $$Promise.resolved(Belt_Option.flatMap(Belt_Option.flatMap(Belt_Option.flatMap(match[0].gitHub, (function (gitHub) {
                                              return gitHub.repository;
                                            })), (function (repo) {
                                          return repo.object_;
                                        })), (function (object_) {
                                      if (typeof object_ === "number" || object_[0] !== -678893856) {
                                        return ;
                                      } else {
                                        var blob = object_[1];
                                        return /* tuple */[
...
function upsertFileContent(client, repoOwner, repoName, branchName, filePath, content, sha, param) {
  return GraphQL$EggheadStatic.mutation(client, GraphQL$EggheadStatic.UpdateFileContentMutation.make(repoOwner, repoName, branchName, filePath, "Updated " + filePath, content, sha, /* () */0), "Error updating file content");
}
...
function submitPr(client, branchName, title, body, editedContent, filePath, sha, username, frontMatter) {
  ...
  var ensureIHaveARepo = $$Promise.map(checkDoIHaveARepo(client, repoName, username), (function (value) {
          if (value) {
            ...
            return $$Promise.map(forkRepo(client, repoOwner, repoName), (function (value) {
                          console.log("Promise value: ", value);
                          return /* () */0;
                        }));
          }
        }));
  var currentFileInfo = $$Promise.flatMap(ensureIHaveARepo, (function (param) {
          var currentMasterShaAndContent = getFileShaAndContent(client, repoName, repoOwner, masterFilePath);
          var branchShaAndContent = getFileShaAndContent(client, repoName, repoOwner, branchFilePath);
          return $$Promise.all2(currentMasterShaAndContent, branchShaAndContent);
        }));
  var prepareSyncedFileStartingContents = $$Promise.flatMap(currentFileInfo, (function (fileInfo) {
          var match = fileInfo[0];
          var needToSyncFileVersions;
          if (match !== undefined) {
            var match$1 = fileInfo[1];
            if (match$1 !== undefined) {
              var match$2 = match;
              var masterSha = match$2[0];
              needToSyncFileVersions = /* tuple */[
                masterSha === match$1[0],
                masterSha,
                match$2[1]
              ];
          ...
          if (needToSyncFileVersions[0]) {
            var match$3 = needToSyncFileVersions[1];
            if (match$3 !== undefined) {
              var match$4 = needToSyncFileVersions[2];
              if (match$4 !== undefined) {
                return upsertFileContent(client, username, repoName, "master", filePath, match$4, match$3, /* () */0);
              } else {
  ...
  return $$Promise.flatMap(prepareSyncedFileStartingContents, (function (result) {
                if (result.tag) {
...
function CourseEditor$ChatHistory(Props) {
  ...
  var commentsEl = Belt_List.toArray(Belt_List.map(comments, (function (comment) {
              ...
              var timeEl = React.createElement("span", {
                  ...
                  }, PullRequestManager$EggheadStatic.timeSince(new Date(comment.createdAt)) + " ago");
...
function CourseEditor$Editor(Props) {
  ...
  var match$7 = OneJwt$EggheadStatic.findGitHubLogin(jwtMe);
  var match$8 = Belt_Option.flatMap(lesson, (function (lesson) {
  ...
  } else if (match$7 !== undefined && lesson !== undefined && match$8 !== undefined) {
    var editPayload = match$8;
  ...
  var tmp$3;
  if (lesson !== undefined) {
    var match$9 = lesson.editPayload;
    if (match$9 !== undefined) {
      var match$10 = match$9;
      var edited = match$10.edited;
      tmp$3 = match$10.transcript === edited ? null : React.createElement("button", {
              style: {
                position: "absolute",
                right: "10px"
              },
  ...
  return React.createElement("div", undefined, React.createElement("div", {
                  ...
                  id: "eggy-editor-container"
                      ...
                      className: "editor-body"
                          ...
                          className: state.chatOpen ? "chat-open" : "chat-closed"
  ...
  if (typeof response === "number") {
    ...
    tmp = React.createElement(PullRequestManager$EggheadStatic.make, {
          ...
          refresh: (function (param) {
              ...
              return Curry._2(executeQuery, undefined, /* () */0);
            }),
          onHide: onHide
        });
  }
...
function CourseEditor$ConversationBubble(Props) {
  ...
  return React.createElement("div", {
            ...
            }, state.isOpen ? React.createElement(CourseEditor$Conversation, {
                    client: client,
                    username: username,
